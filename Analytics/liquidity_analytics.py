...
def _extract_values(entry: Dict[str, Any]) -> Tuple[Optional[float], Optional[float], Optional[float], Optional[float], Optional[str]]:
    ...
    return current, last_week, last_month, start_of_year, status
...
def _quality(
    ...
    if status == "FAILED" or (current is None and last_week is None and last_month is None and start_of_year is None):
        return "FAILED"
    if current is not None and last_week is not None and last_month is not None and start_of_year is not None:
        return "OK"
    return "PARTIAL"
...
def _changes(
    ...
    return {"change_1w": change_1w, "change_1m": change_1m, "change_ytd": change_ytd}
...
def build_liquidity_analytics(raw_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    rrp_current, rrp_last_week, rrp_last_month, rrp_start, rrp_status = _extract_values(rrp_entry)
    tga_current, tga_last_week, tga_last_month, tga_start, tga_status = _extract_values(tga_entry)
    walcl_current, walcl_last_week, walcl_last_month, walcl_start, walcl_status = _extract_values(walcl_entry)

    rrp_changes = _changes(rrp_current, rrp_last_week, rrp_last_month, rrp_start)
    tga_changes = _changes(tga_current, tga_last_week, tga_last_month, tga_start)
    walcl_changes = _changes(walcl_current, walcl_last_week, walcl_last_month, walcl_start)
    ...
    return {
        ...
        "data_quality": {
            "rrp": _quality(rrp_current, rrp_last_week, rrp_last_month, rrp_start, rrp_status),
            "tga": _quality(tga_current, tga_last_week, tga_last_month, tga_start, tga_status),
            "walcl": _quality(walcl_current, walcl_last_week, walcl_last_month, walcl_start, walcl_status),
        },
    }
